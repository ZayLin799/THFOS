                      ...
                      <div class="form-group">
                        <select class="form-select" aria-label="Default select example" id="course" name="course">
                          ...
                          @foreach ($batch_infos as $batch_info)
                            @if (!$batch_info->avaliable_student <= 0 && $batch_info->status == 1)
                              @if($s_info->id == $batch_info->id)
                                <option value="{{$batch_info->id}}" selected>{{ $batch_info->course->coursename }}
                                  <span style="font-size: 16px;"> {{ $s_batch->batch_name }}</span>
                                </option>
                              @else
                                <option value="{{$batch_info->id}}">{{ $batch_info->course->coursename }}
