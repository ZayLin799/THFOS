...
<div class="row d-flex justify-content-center align-items-center">
  <div class="col-lg-8 col-xl-6">
      <div class="card rounded-3">
        <div class="card-body">
                  <h3>Edit Class</h3>
                  <form action="{{route('batch_info.update',$batch_info->id)}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                   <div class="mb-3">
                      ...
                      <div class="form-group">
                        <select class="form-control form-control-md" aria-label="Default select example" name="course_id">
                          ...
                          <option value="{{ $course->id }}"{{$batch_info->course_id == $course->id  ? 'selected' : ''}}>{{ $course->coursename }} </option>
                          @endforeach 
                        </select>
                    ...
                    <div class="mb-3">
                      <label for="course_category_id" class="form-label">Batch</label>
                      <div class="form-group">
                        <select class="form-control form-control-md" aria-label="Default select example" name="batch_id" >
                          @foreach ($batches as $batch)
                          <option value="{{ $batch->id }}"{{$batch_info->batch_id == $batch->id  ? 'selected' : ''}}>{{ $batch->batch_name }} </option>
                          @endforeach 
                        </select>
                    ...
                    <div class="mb-3">
                      ...
                      <input type="number" class="form-control" id="avaliable_student" name="avaliable_student" value="{{$batch_info->avaliable_student}}">
                    ...
                    <div class="mb-3">
                      ...
                      <input type="date" class="form-control" id="time_limit" name="start" value="{{ $batch_info->start}}">
